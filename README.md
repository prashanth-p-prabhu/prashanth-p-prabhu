# 👋 Hi — I'm Prashanth P Prabhu

Platform Engineering & DevSecOps leader with 20+ years building secure, scalable cloud-native systems and growing high-performing engineering teams.

What I do
- Architect enterprise-scale cloud-native platforms, microservices, and resilient infrastructure
- Embed security into the SDLC and CI/CD pipelines (DevSecOps)
- Lead and mentor distributed engineering teams and technical leaders
- Automate infrastructure and delivery using IaC, Kubernetes, and GitOps practices

Core competencies
- Cloud: AWS, GCP, Azure
- Containers & Orchestration: Kubernetes, Docker
- Observability: Prometheus, Grafana, ELK / OpenSearch
- Security: Vault, RBAC, secrets management
- Databases & Storage: PostgreSQL, Redis, MongoDB, Elasticsearch
- Languages & Scripting: Bash, NodeJS, JavaScript

Selected achievements


Leadership philosophy
I focus on empowerment, transparency, continuous learning, automation, and cross-functional collaboration to build reliable systems and healthy teams.

Open to
- Platform Engineering leadership (Director / Senior Director)
- DevSecOps & cloud-native architecture roles
- Advising or mentoring teams on platform transformation

Connect
- GitHub: https://github.com/prashanth-p-prabhu
- LinkedIn: www.linkedin.com/in/prashanth-prabhu-67323125
<a class="libutton" href="https://www.linkedin.com/comm/mynetwork/discovery-see-all?usecase=PEOPLE_FOLLOWS&followMember=prashanth-prabhu-67323125" target="_blank">Follow on LinkedIn</a>
